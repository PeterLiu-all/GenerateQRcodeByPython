...
class QRgenerater:
    ...
    def __parseQR2words(self)->None:
        ...
        if self.__config["if_pay"] == "true":
            qrCodeDetector = cv2.wechat_qrcode_WeChatQRCode('detect.prototxt','detect.caffemodel','sr.prototxt','sr.caffemodel')
            self.__config["words"],points = qrCodeDetector.detectAndDecode(img)
            self.__config["words"] = str(self.__config["words"][0])
        else:
            qrCodeDetector = cv2.QRCodeDetector()
            self.__config["words"],points,_ = qrCodeDetector.detectAndDecode(img)
    ...
    def ActTransform(self)->None:
        ...
        picture= self.__config["picture"] if self.__config["picture"] != "" and os.path.exists(self.__config["picture"]) else None,
        ...
        save_dir=os.getcwd() if self.__config["save_dir"] == "" else self.__config["save_dir"]
    )
def main():
    qr_obj = QRgenerater()
    qr_obj.ActTransform()
